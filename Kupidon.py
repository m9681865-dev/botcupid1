...
cur.execute("""CREATE TABLE IF NOT EXISTS post_admins(
    user_id INTEGER PRIMARY KEY,
    added_by INTEGER,
    added_at TEXT
)""")
...
def is_post_admin(user_id):
    if is_admin(user_id):
        return True
    cur.execute("SELECT user_id FROM post_admins WHERE user_id=?", (user_id,))
    return cur.fetchone() is not None

def add_post_admin(user_id, added_by):
    cur.execute("INSERT OR REPLACE INTO post_admins(user_id, added_by, added_at) VALUES(?, ?, ?)",
                (user_id, added_by, datetime.now().isoformat()))
    db.commit()

def remove_post_admin(user_id):
    cur.execute("DELETE FROM post_admins WHERE user_id=?", (user_id,))
    db.commit()

def get_post_admins():
    cur.execute("SELECT user_id, added_by, added_at FROM post_admins ORDER BY added_at DESC")
    return cur.fetchall()
...
async def probit_post_cmd(message: Message):
    if not is_post_admin(message.from_user.id):
        await message.answer("❌ Доступ запрещен. Только админ или админ постов.")
        return
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
...
async def add_post_admin_cmd(message: Message):
    if message.from_user.id != OWNER_ID:
        await message.answer("❌ Только главный администратор может добавлять админов постов.")
        return
    args = message.text.split(maxsplit=1)
    if len(args) < 2 or not args[1].strip().isdigit():
        await message.answer("❌ Использование: /add_post_admin <user_id>")
        return
    user_id = int(args[1].strip())
    add_post_admin(user_id, message.from_user.id)
    await message.answer(f"✅ Админ постов добавлен.\n🆔 ID: <code>{user_id}</code>", parse_mode="HTML")


@dp.message(Command("remove_post_admin"))
async def remove_post_admin_cmd(message: Message):
    if message.from_user.id != OWNER_ID:
        await message.answer("❌ Только главный администратор может удалять админов постов.")
        return
    args = message.text.split(maxsplit=1)
    if len(args) < 2 or not args[1].strip().isdigit():
        await message.answer("❌ Использование: /remove_post_admin <user_id>")
        return
    user_id = int(args[1].strip())
    remove_post_admin(user_id)
    await message.answer(f"✅ Админ постов удалён.\n🆔 ID: <code>{user_id}</code>", parse_mode="HTML")
...
async def post_admins_cmd(message: Message):
    ...
    if not admins:
        await message.answer("👮 Админов постов пока нет.")
        return
    text = "👮 <b>АДМИНЫ ПОСТОВ</b>\n\n"
...
@dp.callback_query(F.data == "post_admins_menu")
async def post_admins_menu_callback(callback: CallbackQuery):
    ...
    if not admins:
        ...
        for user_id, added_by, added_at in admins:
            text += f"🆔 <code>{user_id}</code> — добавлен {added_at[:16]}\n"
    text += "\n<b>Команды:</b>\n/add_post_admin ID\n/remove_post_admin ID\n/post_admins"
...
async def back_admin_callback(callback: CallbackQuery):
    if callback.from_user.id != OWNER_ID:
        await callback.answer("Нет доступа", show_alert=True)
        return
    await callback.message.edit_text("👑 АДМІН-ПАНЕЛЬ", reply_markup=admin_menu())
    await callback.answer()
